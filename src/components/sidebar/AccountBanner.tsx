...
const AccountBanner: React.FC = () => {
    ...
    return (<Wrapper>
        <Upper_Wrapper>
            <Img_Wrapper><S_img src={LS.userStatus.profileImg}/></Img_Wrapper>
            <UserName_Wrapper>{(() => LS.isLoggedIn? <S_p_2 onClick={clickEventHandler_navigateToUserProfile}>{LS.userStatus.username}</S_p_2> : <S_p_2 onClick={clickEventHandler_navigateToLogin}>Login</S_p_2>)()}</UserName_Wrapper>
        </Upper_Wrapper>
        <Lower_Wrapper >
            {(() => LS.isLoggedIn? (
                <S_div>
